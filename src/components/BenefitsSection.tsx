...
export default function BenefitsSection() {
  return (
    <section className="py-16 px-4" style={{ background: "linear-gradient(180deg, #241a10 0%, #1A120B 100%)" }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <span className="inline-block text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-4"
            style={{ background: "rgba(255, 122, 26, 0.15)", color: "#FFA64D" }}>
            Что вы получите
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {BENEFITS.map((item, i) => (
            <div
              key={i}
              className="rounded-2xl border border-white/10 bg-white/3 p-6 transition-all hover:border-[#FF7A1A]/40 hover:-translate-y-1"
            >
              <div
