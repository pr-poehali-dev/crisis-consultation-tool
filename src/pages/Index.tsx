          ...
          <div className="py-20 px-6 max-w-6xl mx-auto w-full">
            ...
            <div className="mt-20">
              <div className="text-center mb-12">
                <p className="text-neon text-sm uppercase tracking-widest font-semibold mb-2">Реальные истории</p>
                ...
                {[
                  {
                    ...
                    role: "Владелец ресторана «Гастроном»",
                    photo: "https://cdn.poehali.dev/projects/d03b4405-25a0-4b97-9b8f-79e914b22255/files/52f3d285-16ab-416f-884d-e890619093c8.jpg",
                    result: "+18% к прибыли за 5 недель",
                    text: "Думал, что знаю всё про свой бизнес. После аудита с Русланом пересмотрел половину процессов. Нашли утечку на 200к в месяц только на закупках. Реально крутой специалист.",
                    stars: 5,
                  },
                  {
                    name: "Наталья Волкова",
                    role: "Совладелец бара «Сезон»",
                    photo: "https://cdn.poehali.dev/projects/d03b4405-25a0-4b97-9b8f-79e914b22255/files/a36119ad-9c5d-4ff8-978e-978cbb68b8f0.jpg",
                    result: "Вышли из минуса за 2 месяца",
                    text: "Обратились когда бар уходил в минус третий месяц подряд. Руслан провёл полный разбор — меню, персонал, себестоимость. Чёткий план, без лишних слов. Рекомендую всем!",
                    stars: 5,
                  },
                  {
                    name: "Тимур Байсалов",
                    role: "Управляющий сети кофеен",
                    photo: "https://cdn.poehali.dev/projects/d03b4405-25a0-4b97-9b8f-79e914b22255/files/16163c87-8929-4ca1-8f10-159fb3f5494c.jpg",
                    result: "Средний чек вырос на 23%",
                    text: "Запускали новый формат с нуля. Руслан помог выстроить всё с концепции до стандартов. Запустились на месяц раньше срока и сразу вышли в плюс. Незаменимый эксперт.",
                    stars: 5,
                  },
                ].map((r, i) => (
                  <div key={i} className="glass-card rounded-3xl p-6 flex flex-col gap-4 relative overflow-hidden group hover:border-neon/40 transition-all duration-300" style={{ border: "1px solid rgba(255,107,0,0.15)" }}>
                    {/* glow */}
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" style={{ background: "radial-gradient(circle at 50% 0%, rgba(255,107,0,0.06) 0%, transparent 70%)" }} />

                    {/* Результат-бейдж */}
                    <div className="inline-flex items-center gap-2 bg-neon/10 border border-neon/25 rounded-full px-3 py-1 self-start">
                      <span className="w-1.5 h-1.5 rounded-full bg-neon flex-shrink-0" />
                      <span className="text-neon text-xs font-bold uppercase tracking-wide">{r.result}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-white/8">
                      ...
                      <div>
                        ...
                        <p className="text-white/45 text-xs mt-0.5">{r.role}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {/* Плейсхолдер для видеоотзывов */}
              <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { name: "Алексей Морозов", role: "Ресторан «Гастроном»" },
                  { name: "Наталья Волкова", role: "Бар «Сезон»" },
                  { name: "Тимур Байсалов", role: "Сеть кофеен" },
                ].map((v, i) => (
                  <div key={i} className="relative rounded-3xl overflow-hidden aspect-[9/13] flex items-end cursor-pointer group" style={{ background: "linear-gradient(160deg, #1a1a1a 0%, #111 100%)", border: "1px solid rgba(255,107,0,0.15)" }}>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-16 h-16 rounded-full flex items-center justify-center transition-transform group-hover:scale-110 duration-300" style={{ background: "rgba(255,106,0,0.15)", border: "2px solid rgba(255,107,0,0.4)" }}>
                        <Icon name="Play" size={28} className="text-neon ml-1" />
                      </div>
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                    <div className="relative z-10 p-5">
                      <p className="text-white/40 text-xs uppercase tracking-widest mb-1">Видеоотзыв</p>
                      <p className="font-oswald text-white font-bold text-lg uppercase leading-tight">{v.name}</p>
                      <p className="text-white/50 text-xs mt-0.5">{v.role}</p>
                    </div>
                  </div>
                ))}
              </div>
